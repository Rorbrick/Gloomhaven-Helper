...
from wtforms import StringField, PasswordField, BooleanField, SubmitField,SelectField,BooleanField,FieldList, FormField, Form,IntegerField
...
class RegistrationForm(FlaskForm):
    ...
    def validate_name(self, name):
        name = db.session.scalar(sa.select(Character).where(Character.name == name.data))
        if name is not None:
...
def wrapper_func(perk_list,checked): #pass in the perk list
    ...
    for perk in perk_list:
        setattr(PerkForm, str(perk[0]), BooleanField(label=perk[0],default=checked[perk[0]]['checked'])) #setting the booleanfield in the Perk_list class
        if perk[1] > 1:            
            new_perk_name = str(perk[0]) + "_2"
            checked_, disabled = ('checked', 'enabled') if checked[perk[0]]['times_unlocked'] > 1 else (None, 'disabled')

            setattr(PerkForm, new_perk_name, BooleanField(label=perk[0],default=checked_))
