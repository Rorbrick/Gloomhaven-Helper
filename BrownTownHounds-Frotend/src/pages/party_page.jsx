...
import {
  ...
  useCreatePartyNote,
  useDeletePartyNote,
  useCreatePartyAchievement,
  usePartyAchievements,
} from "../api/parties.query";
...
function PartyDetails() {
  ...
  const {
    isLoading: isPartyUpdateLoading,
    error: partyUpdateError,
    mutate: partyMutate,
  } = useUpdateParty(id);
  const {
    data: partyNotes,
    isLoading: isPartyNotesLoading,
    error: partyNotesError,
  } = usePartyNotes(id);
  const {
    isLoading: createPartyNoteIsLoading,
    error: createPartyNoteError,
    mutate: createNoteMutate,
  } = useCreatePartyNote(id);
  const {
    isLoading: createPartyAchievementIsLoading,
    error: createPartyAchievementError,
    mutate: createAchievementMutate,
  } = useCreatePartyAchievement(id);
  const {
    isLoading: deletePartyNoteIsLoading,
    error: deletePartyNoteError,
    mutate: deleteNoteMutate,
  } = useDeletePartyNote(id);
  const {
    data: partyAchievements,
    isLoading: partyAchievementsIsLoading,
    error: partyAchievementsError,
  } = usePartyAchievements(id);

  //Checkers - loading and errors
  const isLoading =
    isPartyLoading ||
    isPartyUpdateLoading ||
    isPartyNotesLoading ||
    createPartyNoteIsLoading ||
    deletePartyNoteIsLoading ||
    partyAchievementsIsLoading ||
    createPartyAchievementIsLoading;
  const error =
    PartyError ||
    partyUpdateError ||
    partyNotesError ||
    createPartyNoteError ||
    deletePartyNoteError ||
    partyAchievementsError ||
    createPartyAchievementError;
