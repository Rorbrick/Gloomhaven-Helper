...
export function useUpdateCharacter(characterId) {
  ...
  return useMutation({
    ...
    onMutate: async (charData) => {
      ...
      const previousCharacterData = queryClient.getQueryData(
        qk.character(characterId),
      );
      const previousCharacters = queryClient.getQueryData(qk.characters());
  ...
  return useMutation({
    ...
    onMutate: async (note_id) => {
      await queryClient.cancelQueries({
        queryKey: qk.characterNotes(characterId),
      });

      const previousCharacterNotes = queryClient.getQueryData(
        qk.characterNotes(characterId),
      );
    ...
    onError: (err, newCharacterNotes, context) => {
      queryClient.setQueryData(
        qk.characterNotes(characterId),
        context.previousCharacterNotes,
      );
    },
    onSettled: () => {
...
export function useUpdateCharacterPerk(characterId) {
  ...
  return useMutation({
    ...
    onMutate: async (perk) => {
      await queryClient.cancelQueries({
        queryKey: qk.characterPerks(characterId),
      });

      const previousCharacterPerks = queryClient.getQueryData(
        qk.characterPerks(characterId),
      );
    ...
    onError: (err, newCharacterPerk, context) => {
      queryClient.setQueryData(
        qk.characterPerks(characterId),
        context.previousCharacterPerks,
      );
    },
    onSettled: () => {
      queryClient.invalidateQueries({
