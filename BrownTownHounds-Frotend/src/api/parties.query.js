...
export function useDeletePartyNote(partyId) {
  ...
  return useMutation({
    ...
    onMutate: async (note_id) => {
      ...
      const previousPartyNotes = queryClient.getQueryData(
        qk.partyNotes(partyId),
      );
...
export function useCreatePartyAchievement(partyId) {
  ...
  return useMutation({
    mutationFn: (newAchievement) =>
      api.createPartyAchievement(partyId, newAchievement),
    onMutate: async (newAchievement) => {
      await queryClient.cancelQueries({
        queryKey: qk.partyAchievements(partyId),
      });

      const previousPartyAchievements = queryClient.getQueryData(
        qk.partyAchievements(partyId),
      );
    ...
    onError: (err, newPartyAchievement, context) => {
      queryClient.setQueryData(
        qk.partyAchievements(partyId),
        context.previousPartyAchievements,
      );
    },
    onSettled: () => {
