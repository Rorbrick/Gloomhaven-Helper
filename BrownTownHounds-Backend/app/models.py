...
class Character(db.Model):
    ...
    char_perk: so.Mapped[list["Character_Perk"]] = so.relationship(back_populates='character', cascade="all, delete", passive_deletes=True)
    class_name: so.Mapped[Class] = so.relationship(back_populates='characters')
    notes: so.WriteOnlyMapped[list["Notes"]] = so.relationship(back_populates='character', cascade="all, delete", passive_deletes=True)
