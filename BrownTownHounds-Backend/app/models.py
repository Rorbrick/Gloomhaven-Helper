...
class Class(db.Model):
    ...
    class_perks: so.Mapped[list["Class_Perk"]] = so.relationship(
        back_populates="class_"
    )
    characters: so.Mapped[list["Character"]] = so.relationship(
        back_populates="class_name"
    )
    retired_characters: so.Mapped[list["RetiredCharacter"]] = so.relationship(
        back_populates="class_name"
    )
...
class Perk(db.Model):
    ...
    char_perk: so.Mapped[list["Character_Perk"]] = so.relationship(
        back_populates="perk"
    )
...
class Character(db.Model):
    ...
    notes: so.Mapped[list["Notes"]] = so.relationship(
        back_populates="character", cascade="all, delete-orphan"
    )
...
class Class_Perk(db.Model):
    ...
    class_id: so.Mapped[int] = so.mapped_column(
        sa.ForeignKey(Class.id), primary_key=True
    )
    perk_id: so.Mapped[int] = so.mapped_column(sa.ForeignKey(Perk.id), primary_key=True)
    times_unlockable: so.Mapped[int] = so.mapped_column(
        sa.Integer, index=True, default=1
    )
...
class Character_Perk(db.Model):
    ...
    character_id: so.Mapped[int] = so.mapped_column(
        sa.ForeignKey(Character.id), primary_key=True
    )
    perk_id: so.Mapped[int] = so.mapped_column(sa.ForeignKey(Perk.id), primary_key=True)
...
class Party(db.Model):
    id: so.Mapped[int] = so.mapped_column(primary_key=True)
    name: so.Mapped[str] = so.mapped_column(sa.String(64), index=True, unique=True)
    location: so.Mapped[str] = so.mapped_column(
    ...
    notes: so.WriteOnlyMapped["PartyNotes"] = so.relationship(
        back_populates="party", cascade="all, delete", passive_deletes=True
    )
    achievements: so.WriteOnlyMapped["PartyAchievements"] = so.relationship(
        back_populates="party", cascade="all, delete", passive_deletes=True
    )
...
class PartyNotes(db.Model):
    id: so.Mapped[int] = so.mapped_column(primary_key=True)
    party_id: so.Mapped[str] = so.mapped_column(sa.ForeignKey(Party.id), index=True)
    text: so.Mapped[str] = so.mapped_column(sa.String(256))
    timestamp: so.Mapped[datetime] = so.mapped_column(
        index=True, default=lambda: datetime.now(timezone.utc)
    )
