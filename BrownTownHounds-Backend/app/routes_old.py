...
def index():
    ...
    if request.method == "POST":
        ...
        if delete_char_id:
            ...
            retired_char = RetiredCharacter(
                name=char.name,
                level=char.level,
                gold=char.gold,
                class_name=db.session.query(Class)
                .filter(Class.id == char.class_id)
                .first(),
            )
    ...
    return render_template(
        "index.html", title="Home", characters=characters, parties=parties
    )  # passing this new dictionary ^ to the index.html page. on that page, they will be displayed.
...
def register():
    ...
    form.class_.choices = [
        (c.name) for c in Class.query.all()
    ]  # adding to a variable the list of class names from the Class DB (Class class in models)
    if form.validate_on_submit():  # once class has been selected, continue
        ...
        char = Character(
            name=form.name.data,
            class_name=db.session.query(Class)
            .filter(Class.name == selected_class)
            .first(),
        )  # create character using the name inputted, and using the class_name relationship in the Character class to find the correct entry in the Class table
    ...
    return render_template(
        "register.html", title="Register Character", form=form
    )  # loads the register.html page initially, passing in the registration from details from the RegistrationForm() class (assigned above)
...
def party(party_name):
    ...
    """
    party_obj = db.first_or_404(
        sa.select(Party).where(Party.name == party_name)
    )  # Get party from database
    party_achievements = db.session.scalars(
        party_obj.achievements.select()
    ).all()  # Get all achievements for this party from database
    party_notes = db.session.scalars(
        party_obj.notes.select()
    ).all()  # Get all notes for this party from database
    ...
    if request.method == "POST":
        ...
        # if you added an achievement, create a new index for this achievement in the PartyAchievement DB table
            ...
            if form.achievement.data:
                new_achievement = PartyAchievements(
                    text=form.achievement.data, party_id=party_obj.id
                )
        ...
        for button_name, model_class in DELETE_HANDLERS.items():
            ...
            if delete_id:
                handle_delete_button(model_class, delete_id)
    ...
    return render_template(
        ...
        form=form,
        party_achievements=party_achievements,
...
def character(character):
    char = db.first_or_404(
        sa.select(Character).where(Character.name == character)
    )  # if the character exists, store the information in the char var, otherwise present 404
    class_perks = db.session.scalars(
        sa.select(Class_Perk)
        .where(Class_Perk.class_id == char.class_name.id)
        .order_by(Class_Perk.perk_id)
    ).all()  # store all class perks for this character
    class_perks_detail = [
        (perk.perk.name, perk.times_unlockable) for perk in class_perks
    ]  # get all perks for character class, then add the names to a list
    char_unlocked_perks = [
        (cp.perk.name, cp.times_unlocked) for cp in char.char_perk
    ]  # adds all unlocked perks (found in char_perk table) to a set
    SetCharacterLevel(char.xp, char)
    char_notes = db.session.scalars(char.notes.select()).all()

    perks_form = wrapper_func(
        class_perks_detail, active_perk_checker(class_perks_detail, char_unlocked_perks)
    )  # send in the class perks to the wrapper function, which will create a checkbox for each. Also send in active perks, and the number of times they've been unlocked

    charStats = CharacterDetailsForm(request.form, obj=char)
    perkForm = perks_form(
        request.form if request.method == "POST" else None
    )  # assigning the perks_form to form
    ...
    if request.method == "POST":
        ...
        if perkForm.validate():
            handle_perk_form_submission(
                perkForm, char, char_unlocked_perks
            )  # pdate unlocked perks

        if perkCheckboxes.validate():
            handle_perk_points_submission(
                char, perkCheckboxes
            )  # update perk points (the checkmarks that indicate if you can unlock a new perk)

        # delete note if delete button was pressed
        delete_id = request.form.get("delete_note_id")
        if delete_id:
            handle_delete_button(Notes, delete_id)
...
def handle_perk_form_submission(form, char, char_unlocked_perks):
    ...
    for checkbox in form:
        perk_is_unlocked = next(
            (perk for perk in char_unlocked_perks if perk[0] == checkbox.name), None
        )
        was_checked = (
            next(
                (
                    perk
                    for perk in char_unlocked_perks
                    if perk[0] == checkbox.label.text
                ),
                None,
            )
...
def active_perk_checker(class_perks_detail, unlocked_perks_detail):
    ...
    for class_perk_name, _ in class_perks_detail:
        matched_unlocked = next(
            (
                unlocked
                for unlocked in unlocked_perks_detail
                if unlocked[0] == class_perk_name
            ),
            None,
        )
        ...
        if matched_unlocked:
            ...
            checkbox_states[class_perk_name] = {"checked": None, "times_unlocked": 0}
...
def unlock_perk(checkbox, existing_perk, character):
    ...
    if is_second_unlocked:
        if existing_perk:
            ...
            add_character_perk(
                character, perk, 1
            )  # if the user checked the second checkbox, and the first is unchecked, instead, add the perk to the char_perk table. Upon reload, first checkbox is checked
    else:
...
def add_character_perk(character, perk, times_unlocked):
    ...
    new_char_perk = Character_Perk(
        character=character, perk=perk, times_unlocked=times_unlocked
    )
